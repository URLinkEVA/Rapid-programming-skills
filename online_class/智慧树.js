// ==UserScript==
// @name         ❤❤❤❤【智慧树】[最新版]❤智慧树知到❤❤网课辅助题库庞大❤❤❤全自动化操作挂机就完事❤❤❤❤
// @namespace    无名
// @version      1.3.0.1
// @description  ❤❤❤❤可以隐藏搜索框小卡片，后台操作无情哈了哨用就完事儿自动挂机爽爽的看，支持屏蔽弹窗题目、自动切换下一节，章测试和考试支持自动答题，视频自动倍速播放、默认静音等，解除各类功能限制，开放自定义参数 ，可以自行配置运行，辅助首选！可以隐藏搜索框小卡片，后台操作
...
var setting = {
    ...
    setting.notice="niswodeyan<h1>ddasdasdasdaskkasfjhciuwheciahkjahsjdhakuiukisajdhuhcjszjadjhajhdhqhdh"
...
function beforeFind() {
    ...
    setting.div = $(
        '<div style="border: 2px dashed rgba(138, 243, 247,0.7); width: 330px; position: fixed; top: 0; left: 0; z-index: 99999; background-color: rgba(197, 252, 255,0.7); overflow-x: auto;;border-radius:10px">' +
        '<span style="font-size: medium;"></span>' +
        '<label style="font-weight: "></label>' +
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.answer) {
                    ...
                    setting.div.children('div:eq(0)').html(obj.data || '服务器繁忙，无限调用...请联系：VX:HH54068');
                }
               
                setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                setting.div.children('div:eq(0)').data('html', '请求过于频繁，无限调用请联系：vx:HH54068').siblings('button:eq(0)').click();
            } else {
                setting.div.children('div:eq(0)').text('服务器异常，无限调用...请联系：VX:HH54068');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').text('服务器超时，无限调用...请联系：VX:HH54068');
        }
    });
}
